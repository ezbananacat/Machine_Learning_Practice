...
def start_predict_xgboost():
    ...
    to_drop = final_merged.columns[final_merged.isnull().sum() > missing_threshold]

    final_merged_cleaned = final_merged.drop(columns=to_drop)

    
    # final_merged.to_csv('test_data.csv', index=False)
    final_merged_cleaned = final_merged.dropna()
    final_merged_cleaned.to_csv('real_merged_data2.csv', index=False)
    # You code above:
